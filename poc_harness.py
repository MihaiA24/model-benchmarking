#!/usr/bin/env python3
"""Harness PoC single-shot para spring-petclinic (bugs PetValidator + OwnerController)."""
import time, shutil, subprocess, csv, pathlib, re, requests, stat, os

# La API key se lee de un fichero local (no de variable de entorno).
KEY_FILE = pathlib.Path("openrouter_key.txt")
OPENROUTER_API_KEY = KEY_FILE.read_text().strip() if KEY_FILE.exists() else ""
if not OPENROUTER_API_KEY or OPENROUTER_API_KEY == "PEGA_AQUI_TU_KEY":
    raise SystemExit("Falta la API key: pega tu key de OpenRouter en openrouter_key.txt")

OPENROUTER_URL = "https://openrouter.ai/api/v1/chat/completions"

# VERIFICA los slugs exactos en https://openrouter.ai/models
MODELS = [
    "minimax/minimax-m3",
    "deepseek/deepseek-v4-flash",
    "z-ai/glm-4.7",
]
PRICES = {  # $/1M tokens (entrada, salida)
    "minimax/minimax-m3":        (0.30, 1.20),
    "deepseek/deepseek-v4-flash": (0.09, 0.18),
    "z-ai/glm-4.7":              (0.40, 1.75),
}
RUNS = 3
BASELINE = "baselines/petclinic"

TASKS = [
    {
        "name": "bug1-petvalidator",
        "target_file": "src/main/java/org/springframework/samples/petclinic/owner/PetValidator.java",
        "prompt": (
            "Reporte de bug: al validar una mascota, un nombre formado solo por "
            "espacios en blanco (espacios, tabuladores, saltos de linea) se acepta "
            "como valido, cuando deberia rechazarse como obligatorio/vacio. "
            "Corrige la validacion en este fichero, respetando el estilo del proyecto. "
            "Devuelve SOLO el contenido completo y corregido del fichero, en un unico "
            "bloque de codigo."
        ),
        "build_cmd": ["mvn", "-q", "-DskipTests", "compile"],
        "test_cmd":  ["mvn", "-q", "-Dtest=PetControllerTests", "test"],
    },
    {
        "name": "bug2-ownercontroller",
        "target_file": "src/main/java/org/springframework/samples/petclinic/owner/OwnerController.java",
        "prompt": (
            "Reporte de bug: en la busqueda de propietarios, cuando hay varios "
            "resultados la aplicacion redirige directamente al detalle de un "
            "propietario en lugar de mostrar la lista paginada. Correcto: un unico "
            "resultado va al detalle; varios resultados muestran la lista. Corrigelo "
            "en este fichero, respetando el estilo del proyecto. Devuelve SOLO el "
            "contenido completo y corregido del fichero, en un unico bloque de codigo."
        ),
        "build_cmd": ["mvn", "-q", "-DskipTests", "compile"],
        "test_cmd":  ["mvn", "-q", "-Dtest=OwnerControllerTests", "test"],
    },
]

RESULTS = pathlib.Path("results"); RESULTS.mkdir(exist_ok=True)
CSV_PATH = RESULTS / "metrics.csv"


def call_model(model, file_content, prompt):
    user_msg = f"{prompt}\n\n--- FICHERO ACTUAL ---\n{file_content}"
    t0 = time.time()
    resp = requests.post(
        OPENROUTER_URL,
        headers={"Authorization": f"Bearer {OPENROUTER_API_KEY}"},
        json={"model": model,
              "messages": [{"role": "user", "content": user_msg}],
              "temperature": 0.2},
        timeout=300,
    )
    resp.raise_for_status()
    data = resp.json()
    latency = time.time() - t0
    text = data["choices"][0]["message"]["content"]
    return text, data.get("usage", {}), latency


def extract_code(text):
    m = re.search(r"```(?:\w+)?\n(.*?)```", text, re.S)
    return m.group(1) if m else text


def _force_remove(func, path, _):
    os.chmod(path, stat.S_IWRITE)
    func(path)


def run_checks(workdir, task):
    out = {}
    for label, cmd in [("build", task["build_cmd"]), ("test", task["test_cmd"])]:
        p = subprocess.run(" ".join(cmd), cwd=workdir, capture_output=True, text=True, shell=True)
        out[label] = (p.returncode == 0)
    return out


def main():
    write_header = not CSV_PATH.exists()
    with open(CSV_PATH, "a", newline="") as f:
        w = csv.writer(f)
        if write_header:
            w.writerow(["task", "model", "run", "build_ok", "test_ok",
                        "in_tok", "out_tok", "cost_usd", "latency_s", "workdir"])
        for task in TASKS:
            for model in MODELS:
                for run in range(1, RUNS + 1):
                    workdir = RESULTS / f"{task['name']}__{model.replace('/', '_')}__r{run}"
                    try:
                        if workdir.exists():
                            shutil.rmtree(workdir, onerror=_force_remove)
                        shutil.copytree(BASELINE, workdir)
                        target = workdir / task["target_file"]
                        text, usage, latency = call_model(model, target.read_text(), task["prompt"])
                        target.write_text(extract_code(text))
                        checks = run_checks(workdir, task)
                        it = usage.get("prompt_tokens", 0); ot = usage.get("completion_tokens", 0)
                        pin, pout = PRICES.get(model, (0, 0))
                        cost = it / 1e6 * pin + ot / 1e6 * pout
                        (workdir / "_raw_response.txt").write_text(text)
                        w.writerow([task["name"], model, run, checks["build"], checks["test"],
                                    it, ot, round(cost, 4), round(latency, 1), str(workdir)])
                        print(f"{task['name']:22} | {model:32} | run {run} -> "
                              f"build={checks['build']} test={checks['test']} ${cost:.4f}")
                    except Exception as e:
                        w.writerow([task["name"], model, run, "ERROR", "ERROR", 0, 0, 0, 0, str(e)[:200]])
                        print(f"{task['name']:22} | {model:32} | run {run} -> ERROR: {e}")
    print(f"\nListo. Metricas en {CSV_PATH}. Anonimiza 'model' antes de puntuar.")


if __name__ == "__main__":
    main()
