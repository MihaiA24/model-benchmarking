...
One independent reviewer confirms that the Developer Brief and verifier specify the same observable behavior, required checks do not encode an undisclosed implementation preference, provenance and licensing are complete, the work is professionally plausible and bounded for its declared difficulty, and the agent-visible package contains no answer leakage. The benchmark owner designates a reviewer who did not author or implement the Scenario and has access to every artifact required for its visibility class; an agent, package author, or unavailable reviewer cannot self-approve the gate.

Seal the decision as `scenario-review.json` beside the visibility-scoped qualification evidence. Its strict schema records the review-schema and checklist versions; Scenario, Verifier, Score Contract, package-lock, and payload identities; a non-secret reviewer identity and independence attestation; review timestamp; one `approve` or `reject` judgment with reasons for brief/verifier alignment, implementation neutrality, professional realism, difficulty/boundedness, provenance/licensing, answer leakage, and disclosure handling; and the reviewer's authenticated signature or repository identity. A missing, conflicted, unsigned, inaccessible, or rejected review blocks Package Qualification. Rejections remain durable evidence and replacement may occur only under the accepted same-cell policy; no automated score substitutes for human approval.

Lock generation precedes the review record and Package Qualification Record. Any subsequent content or component-identity change invalidates both and requires qualification and independent review again. The Package Qualification Record is independent of Harness, adapter, Provider Route, model, Worker Profile, or Production Experiment Manifest. Those exact-condition controls are proven later by experiment-owned Qualification Bundles.
