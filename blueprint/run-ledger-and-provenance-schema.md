...
3. **Ledger Amendments** record later replacement designation, correction, supersession, or invalidation by pointing to prior records. They never rewrite or delete those records.
...
First, structurally omit secrets at their source. Persist only allowlisted environment-variable names, never unrestricted environment dumps. Strip authentication headers, credential-bearing query parameters, URL userinfo, signed locator parameters, and secret-valued configuration fields. Record only each secret's logical name, purpose, scope, host-side Credential Proxy injection boundary, and redaction treatment. There is no agent-exposed provider-credential mode. Never persist secret values, access-granting provider token identifiers, or reusable hashes of secrets. Storage locators are unsigned stable references. The Credential Proxy records metadata and usage rather than request or response payloads solely for surveillance.
