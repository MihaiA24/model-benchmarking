...
Run **50,000 deterministic simulations per candidate and Analysis Stratum**. Within each Scenario, resample pilot Matched Block records while preserving the full three-Harness outcome vector for complete blocks and the terminal loss state for incomplete blocks, then execute the same frozen interval, eligibility, and completeness code used for production. Never resample Harness conditions independently. Record the deterministic seed, simulation and analysis implementation digests, environment and dependency identities, candidate pass rates, and selected count in the analysis manifest and sealed selection artifact.
...
- [Define the run ledger and provenance schema](https://github.com/MihaiA24/model-benchmarking/issues/21) records the selected repetition ordinal, analysis-manifest identity, Pricing Records, Planned Trial Cells, replacement lineage, effective complete-block denominators, and a reference to the sealed candidate-selection artifact without storing report-only simulations as Trial facts.
