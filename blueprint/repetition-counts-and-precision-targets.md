...
A single outer simulation satisfies the precision target only when, after applying the accepted completeness code, **all three pairwise Harness contrasts simultaneously** produce available, valid intervals and each exact equal-tailed interval has `(upper − lower) / 2 <= 0.20`. Boundary equality passes. An unavailable, invalid, or over-width interval for any one contrast makes the whole simulation a failure. A candidate satisfies one Analysis Stratum only when at least 40,000 of the same 50,000 outer simulations pass this joint three-contrast predicate; three separate marginal 80% pass rates are insufficient.
...
Before sealing the Production Experiment Manifest, run exactly **50,000 deterministic whole-campaign aggregate-spend simulations** for the complete precision-qualified candidate schedule under the frozen model roster. Build one pilot **cell-fulfilment cost episode** for every Planned Trial Cell: sum proxy-observed provider usage across the original Trial Attempt, all Harness-native requests and retries, and the one policy-permitted replacement when designated. Reprice every request with the immutable Pricing Record applicable to production. A `not_started` attempt with no provider request contributes zero; every request made by a valid, invalid, aborted, or replaced attempt still contributes its actual repriced cost. If a replacement-eligible original attempt has no terminal replacement decision, or any request usage or applicable price is unavailable, the spend input is incomplete and design qualification fails rather than imputing cost.

Group the three Harness cost episodes and their terminal states into the same pilot Matched Block identity used by precision sizing. For each candidate production repetition of each exact Scenario × exact-model cell, draw one of that Scenario and model's three pilot block vectors with replacement. Preserve the complete three-Harness vector; never sample Harness costs independently, pool Public and Private Scenarios, exchange observations across Scenarios or models, or discard analysis-incomplete blocks from spend. Use independent counter-derived draws for distinct Scenario × model × production-repetition positions, keyed by the frozen root seed, `aggregate_spend`, simulation ordinal, Suite, Workload Family, Scenario, exact model, and production repetition. This preserves observed within-block Harness dependence while retaining the frozen Scenario, Suite, workload, and model structure.

For each simulation, sum all three Harness episode costs over every planned production block in the complete candidate schedule: the count selected for each Workload Family × exact-model profile applies to its three Public and three Private Scenarios, and totals are then summed across the entire frozen model roster. Pilot spend is excluded from this production total and remains governed by its separate $1,000 ceiling. Publish golden vectors covering complete, valid-limit, invalid, aborted, zero-request `not_started`, and designated-replacement episodes, plus schedule expansion and whole-roster summation.

Use the same counter-based SHA-256 sampler and define the empirical 95th percentile as the 47,500th value in the nondecreasing 50,000-value production-spend vector—the smallest value whose cumulative proportion is at least `0.95`. The complete design qualifies only when that value is at or below $4,000. Boundary equality passes; unavailable or incomplete spend evidence fails qualification. If no qualifying complete design fits, reduce the pre-production model roster or obtain explicit budget reauthorization and seal a new Production Design Selection and manifest. Do not weaken margins, selectively omit expensive cells, begin an unaffordable design, or treat an aggregate-spend stop as publishable evidence. Reaching the ceiling before the locked design completes leaves the comparison incomplete and non-publishable under the accepted protocol.
