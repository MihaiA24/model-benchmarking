...
Trusted provisioning is an explicit producer boundary, not an operator convention. `runtime` compiles a sealed provisioning plan from `declarations`; `experiment provision` builds or imports the project wheel, coordinator image, Scenario runtime images, verifier images, and one Harness artifact image per Stock Profile in trusted network-enabled provisioning; resolves every OCI and source input to its declared digest; places Public/Calibration and Private bytes in physically separate content-addressed cache roots; and seals a Provisioning Manifest plus build/qualification evidence. `experiment preflight` is read-only: it verifies the Provisioning Manifest, cache-root policy, image availability, and digests and fails before any pull, build, import, or measured launch.
...
1. **`declarations`** owns Scenario Package validation and locks, `standard-v1` expansion, Suite Releases, Package Qualification Records, compatibility and lifecycle records, Production Experiment Manifests, Planned Trial Cells, Worker Qualification Records, canonical serialization, and typed digests. It emits immutable declarations for later modules and refuses to seal production without an accepted Production Design Selection.
2. **`runtime`** owns trusted provisioning plans, the three Harbor Adapters, single-cell Harbor job compilation, Credential Proxy process, preflight, launch, external monitoring, cumulative-spend admission, and Harbor child-process execution. It consumes one sealed Planned Trial Cell and freezes one completed raw attempt workspace against further runtime writes, containing Harbor-native and host-side evidence. It does not classify task quality, amend the Run Ledger, or calculate report statistics.
3. **`evidence`** owns the trusted capture implementation and contract, Final Repository Capture, Submission derivation and validation, redaction, Artifact Descriptors, Result Bundle sealing, Trial Attempt disposition reconciliation, append-only Run Ledger writes and Amendments, and read-back verification. Its immutable capture collector is packaged into the qualified post-stop sidecar seam used by `runtime`; after Harbor completes, `evidence` revalidates the collector record and converts the Planned Trial Cell plus raw attempt workspace into sealed canonical evidence.
4. **`analysis`** owns deterministic pre-production sizing and aggregate-spend qualification, Production Design Selections, effective Amendment and replacement projections, disposition-aware analysis datasets, paired statistics, compatibility gates, and generation and sealing of the static report. It has read-only access to canonical Trial evidence.
...
Trusted Submission Capture is the one required pre-verifier interposition. Pinned Harbor must stop the main Harness service and its descendants before invoking the immutable capture sidecar's collect hook; that hook reads only the fresh Trial-local repository volume, derives and validates the normalized patch or no-op/non-patch artifact, and exposes only the accepted handoff for Harbor's native separate-verifier upload. The Harness cannot write the collector image or post-stop output, and no whole workspace crosses. If the real-Docker proof cannot establish those properties through the supported sidecar seam, fail qualification and reopen the bounded substrate fallback; do not move capture after verification or introduce a second verifier lifecycle.

Keep Angular and TypeScript, Spring Boot and Java, and Python data-engineering support as declarative templates and data under `scaffolds/`, not executable stack-specific modules. The Credential Proxy is the only project-owned network service, scoped to the provider route used by a Trial; the trusted capture collector is an ephemeral Harbor-managed sidecar with no independent lifecycle or network service. Private implementation submodules are allowed, but they do not enlarge the four public module interfaces.
...
Because the initial evaluator is strictly sequential, enforce aggregate provider spend without a concurrent reservation service. Before each pilot or production launch, derive the authoritative cumulative spend from sealed effective proxy-usage records and immutable Pricing Records, reconcile any prior unsealed attempt before proceeding, subtract it from the applicable campaign ceiling, and configure the Trial proxy with the lesser of the cell ceiling and verified remaining campaign budget. Unavailable usage/pricing, a non-positive remainder, or ambiguous prior state blocks launch. Replacement Trials consume the same campaign total. Reaching a campaign ceiling makes the frozen design incomplete and non-publishable; it never authorizes outcome-based omission or a silent budget increase.
...
Before production sealing, `experiment qualify-design` consumes the frozen pilot Run Ledger, Suite-owned analysis semantics, exact model roster, Pricing Records, and pinned analysis implementation. It runs the accepted repetition and aggregate-spend simulations and seals one Production Design Selection containing candidate pass rates, selected counts, completeness evidence, spend quantiles, seeds, implementation/environment identities, and input digests. `experiment seal` accepts that artifact only when every required stratum qualifies and rejects a stale, incompatible, incomplete, or unaffordable selection.
...
- `worker qualify`;
- `experiment provision`, `experiment qualify-design`, `experiment seal`, `experiment preflight`, and `experiment run`;
...
2. **Harbor contract tests** load generated Scenario and single-cell job configurations with pinned Harbor and prove custom Adapter import paths, one-Trial expansion, lock contents, separate-verifier configuration, selected-only read-only Harness image mounts, native result ingestion, and artifact-manifest handling. They attempt writes to the mounted Harness artifact, assert that unselected Harness images and verifier bytes are absent, and verify Public/Calibration versus Private cache-root segregation. A static architecture test rejects project imports of Harbor's internal `Job`, `Trial`, queue, and lifecycle modules.
3. **Runtime fault-injection qualification** exercises each Harness through the Credential Proxy and proves no measured download, exact model routing, fresh writable state, hidden-marker absence, complete process-tree termination, the trusted post-stop capture-sidecar handoff, resource and per-Trial/campaign spend limits, unsafe Submission rejection, missing-evidence invalidation, secret redaction, teardown quarantine, and digest read-back. A live long-running fixture proves every resolved Harbor/Adapter wall-clock timeout is disabled while CPU, memory, process, storage, provider, and handoff limits remain enforced. An explicit authenticated operator-abort request records its request identity and monotonic receipt time, terminates the process tree, and seals `aborted_operator`; coordinator interruption without that request remains infrastructure interruption, never an inferred operator abort. The layer also produces and validates Worker Qualification Records, rejects personal credentials/cloud metadata/unrelated host mounts or workloads, and makes `development_only` exclusion unavoidable. It covers infrastructure, Harness, limit, integrity, malformed-output, and operator-abort paths.
